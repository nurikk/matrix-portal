...
`MATRIX_BENCHMARK`. Geometry and the benchmark/board selection are compile-time and require a rebuild + reflash; the runtime simulation knobs are tunable via the WiFi control panel (ESP32-S3 only — see below).
...
- **The Life code is one translation unit, split across headers.** The `src/life_*.h`
  ...
  include order** in `main.cpp` (state → util → profile → color → render → clock → input →
  spawn → sim): e.g. `life_profile.h` must precede `life_render.h` because `renderFrame`
  calls `addProfile`. Staying one TU is deliberate — `-O3` with no LTO still inlines across the
...
- **RAM ceiling.** Per-cell state is ~13 bytes/cell (the `cell*`/`next*`/`visual*` arrays
  in `src/life_state.h`). 128×128 = ~208 KB — fits the S3, **overflows the M4's
...
- **Two cores, one `gLive`.** AsyncTCP is pinned to **core 0** via
  ...
  deferred actions (reseed, clear, pause/resume, forget-wifi reboot) — `gLive` is
...
- **`life_state.h`** — the data model: Life constants, the `#error` geometry guard,
  ...
  (`gLive`/`gSaved`/`gDefaults`), and **all** cell-state / sim / accel / timing /
...
- **`life_util.h`** — low-level numeric / RNG / motion-glow helpers (`popcount64`, `ctz64`,
  `random32`, `absDiff16`, `wrapHue`, `raiseMotionGlow`, …).
...
- **`life_clock.h`** — scheduled/manual wall-clock animations. Minute events use the digital
  clock face; hour events use the analog face. S3 only; non-WiFi builds compile stubs.
- **`life_input.h`** — `pollAccelerometer`, knock detection, `initAccelerometer`,
  `decayMotionEffects` (tap glow fade). Knock triggers the digital minute clock animation on
  S3; tilt/shake were removed.
...
- **Runtime knobs** (`src/life_settings.h`) — simulation parameters (spawn rates,
  density thresholds, hue weights, etc.) in a `LifeSettings` struct generated by the
...
- **Web portal** (`src/web_portal.cpp`, `src/web_ui.h`, S3 Life only) — BLE WiFi
  ...
  promotes them into `gLive` and handles deferred actions (reseed, forget-wifi
